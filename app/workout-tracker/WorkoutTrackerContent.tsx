...
export default function WorkoutTrackerContent() {
  ...
  const getSupabase = () => { if (!supabaseRef.current) supabaseRef.current = createClient(); return supabaseRef.current; };
  ...
  const submit = async () => {
    const sb = getSupabase();
    if (!sb) return;
    await sb.from('workout_logs').insert({ exercise: selected, date, weight: selectedType === 'weighted' ? (+weight || 0) : null });
    setWeight('');
    setPage(0);
    reload();
  };

  const remove = async (id: string) => { const sb = getSupabase(); if (!sb) return; await sb.from('workout_logs').delete().eq('id', id); setPage(p => Math.max(0, p)); reload(); };
