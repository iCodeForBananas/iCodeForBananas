...
export default function Home() {
  ...
  return (
    <div className='flex flex-col flex-1 min-h-0 bg-surface-base'>
      <BentoPageLayout title='iCodeForBananas' titleClassName='text-primary-text'>
        <div className='grid grid-cols-3 sm:grid-cols-5 auto-rows-fr flex-1 min-h-0 w-full gap-2'>
          {tools.map(({ href, text }, index) => (
            <Fragment key={href}>
              {index === bananaIndex && (
                <div
                  className='w-full h-full flex items-center justify-center text-center select-none text-7xl sm:text-8xl leading-none rounded-none bg-primary-hover'
                  aria-hidden='true'
                >
                  🍌
                </div>
