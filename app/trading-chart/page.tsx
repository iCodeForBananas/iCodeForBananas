...
export default function TradingChartPage() {
  ...
  useEffect(() => {
    ...
    setPositions((prev) => {
      ...
      const updated = prev.map((pos) => {
        ...
        const currentStop = pos.currentStopLoss ?? pos.stopLoss ?? currentCandle.ema!;
        ...
        // Trail stop based on EMA, but only in favorable direction
        if (pos.side === PositionSide.LONG) {
          // For long: only move stop UP (trail up towards price)
          if (currentCandle.ema! > currentStop) {
            newStopLoss = currentCandle.ema!;
          }
        } else {
  ...
  const openPosition = (side: PositionSide) => {
    ...
    if (!currentCandle?.ema) {
      alert("EMA line not available yet.");
      return;
    }
    ...
    const size = calculatePositionSize(entryPrice, currentCandle.ema);
    ...
    const newPosition: Position = {
      ...
      size,
      entryTime: Date.now(),
      stopLoss: currentCandle.ema, // Initial stop loss (defines 1R)
      currentStopLoss: currentCandle.ema, // Trailing stop (will update)
      status: "open",
    };
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Enter" && !isLoading) {
        ...
        if (openPositions.length > 0) {
          ...
          // Open new position based on price vs EMA
            ...
            <div className='rounded-lg border border-border bg-white p-4 shadow-sm'>
              <div className='flex items-center justify-center h-[50vh]'>
                <div className='text-center'>
                  ...
                  <p className='text-[#909296]'>Loading SPY data...</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className='rounded-lg border border-border bg-white p-4 shadow-sm'>
              <div className='flex items-center justify-center h-[50vh]'>
                <div className='text-center'>
                  ...
                  <p className='text-[#909296] mb-4'>{error}</p>
                  <button
        ...
        <div className='rounded-lg px-2 sm:px-4 py-2 sm:py-3 shadow-sm'>
          ...
          <div className='bg-slate-50 rounded-lg p-2 sm:p-4 mb-2 sm:mb-4 border border-border'>
            <div className='flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4'>
              <label className='text-xs sm:text-sm font-semibold text-[#F8F9FA]'>Dataset:</label>
              <select
                value={selectedFile}
                onChange={(e) => setSelectedFile(e.target.value)}
                disabled={isLoading || availableDatasets.length === 0}
                className='px-3 py-2 text-xs sm:text-sm font-medium rounded-md border border-gray-300 bg-white text-[#F8F9FA] hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-cyan-500 disabled:opacity-50 disabled:cursor-not-allowed'
              >
          ...
          <div className='grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4'>
            <div className='bg-slate-50 rounded-lg p-2 sm:p-4 border border-border'>
              <h2 className='text-xs sm:text-sm font-semibold text-[#909296] mb-1 sm:mb-2'>Balance</h2>
            ...
            <div className='bg-slate-50 rounded-lg p-2 sm:p-4 border border-border'>
              <h2 className='text-xs sm:text-sm font-semibold text-[#909296] mb-1 sm:mb-2'>Value</h2>
              <p className='text-sm sm:text-xl lg:text-2xl font-bold text-gray-900 truncate'>
            ...
            <div className='bg-slate-50 rounded-lg p-2 sm:p-4 border border-border'>
              <h2 className='text-xs sm:text-sm font-semibold text-[#909296] mb-1 sm:mb-2'>P&L</h2>
              <p
            ...
            <div className='bg-slate-50 rounded-lg p-2 sm:p-4 border border-border'>
              <h2 className='text-xs sm:text-sm font-semibold text-[#909296] mb-1 sm:mb-2'>Buy & Hold</h2>
              <p className='text-sm sm:text-xl lg:text-2xl font-bold text-cyan-600'>
                ...
                {(() => {
                  ...
                  // Find the first candle with valid EMA (starting point)
                  const startingIndex = allData.findIndex((d) => d.ema && d.ema > 0);
                  ...
                  const stopLoss = startCandle.ema!;
          ...
          {openPositions.length > 0 && (
            <div className='mt-2 sm:mt-4 bg-slate-50 rounded-lg p-2 sm:p-4 border border-border'>
              ...
              {openPositions.map((pos) => {
                const currentEma = allData[visibleIndex]?.ema || pos.stopLoss || 0;
                return (
                  <div
                    key={pos.id}
                  ...
                  >
                    <div>
                      <span className='text-[#909296]'>Side:</span>
                      <span
