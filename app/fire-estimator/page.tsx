...
function calcProjection(
  ...
  const empty = {
    ...
    retireYearIdx: -1,
    fireNumber: expenses * FIRE_MULTIPLIER,
    fireType: getFireType(expenses),
    savingsRate: 0,
    coastFireAge: null as number | null,
    baristaFireAge: null as number | null,
  };
  ...
  for (let y = 0; y <= totalYears; y++) {
    ...
    data.push({
      ...
      fireTarget: Math.round(inflatedFireNumber),
    });
  }
        ...
        <div className='rounded-lg p-6 bg-white'>
          <div className='text-center mb-10'>
            <h1
              ...
              style={{ color: "#000" }}
              title='Find out when you can retire early by saving aggressively and investing — the younger you start and the more you save, the sooner you can stop working'
            >
              🔥 FIRE Estimator
            </h1>
            <p className='text-lg mt-3' style={{ color: "#000" }}>
              Rule of 25 · 4% Safe Withdrawal Rate · When can you reach financial independence?
            </p>
          </div>
          ...
          <div className='flex flex-wrap gap-6 mb-6'>
            <label
              ...
              title='Starting earlier means more years of compound growth — even a few years makes a big difference in when you can retire'
            >
            ...
            <label
              className='flex flex-col gap-1'
              title='How long your money needs to last — retiring at 35 with a life expectancy of 90 means 55 years of withdrawals'
            >
            ...
            <label
              className='flex flex-col gap-1'
              title='Higher income means more you can save each year — the gap between this and your expenses is what gets invested toward retirement'
            >
            ...
            <label
              className='flex flex-col gap-1'
              title='Money you already have invested — a head start that compounds and can shave years off your retirement date'
            >
            ...
            <label
              className='flex flex-col gap-1'
              title='The single biggest lever — cutting expenses lowers your FIRE number AND increases how much you save. Reducing by $500/mo can move retirement up by years'
            >
            ...
            <label
              className='flex flex-col gap-1'
              title='With Barista FIRE, part-time income fills the gap so you can quit your career years before full FIRE — even a small side gig helps'
            >
            ...
            <label
              className='flex flex-col gap-1'
              title='401k contributions come from pre-tax income and get invested — maxing this out accelerates your retirement date'
            >
              ...
              <input
                ...
                step='100'
                value={monthly401k}
                onChange={(e) => setMonthly401k(Number(e.target.value))}
                className='border border-[#373A40]/30 rounded px-3 py-2 w-36 bg-white'
              />
            </label>
            <label
              className='flex flex-col gap-1'
              title='Free money from your employer — a 50% match means every $1 you contribute adds $1.50 toward retirement'
            >
            ...
            <label
              className='flex flex-col gap-1'
              title='Your employer only matches up to this % of your salary — contribute at least this much to get the full free money'
            >
              ...
              <input
                ...
                step='1'
                value={matchCapPct}
                onChange={(e) => setMatchCapPct(Number(e.target.value))}
                className='border border-[#373A40]/30 rounded px-3 py-2 w-28 bg-white'
              />
            </label>
            <label
              className='flex flex-col gap-1'
              title='Inflation raises your cost of living every year — higher inflation means you need a bigger portfolio to retire, pushing your date out'
            >
            ...
            <div className='flex flex-col gap-1 justify-end'>
              <label
                className='flex flex-col gap-1'
                title='Investing in index funds grows your money faster — SPY or QQQ can cut years off your retirement date vs keeping cash'
              >
                <span className='text-sm font-semibold'>Invest In</span>
                <select
                  value={investIn}
                  onChange={(e) => setInvestIn(e.target.value)}
              ...
              <span
                ...
                title="The higher your savings rate, the sooner you retire — at 50% you're working one year for every year of freedom, at 75% you retire in roughly 7 years"
              >
              ...
              {returnRate > 0 && (
                <span
                  className='text-sm text-muted'
                  title='Your investments compound at this rate each year — the difference between 10% and 13% can move your retirement date by several years'
                >
                  {investIn.toUpperCase()} avg: {(returnRate * 100).toFixed(1)}% nominal
                </span>
              )}
            </div>
          </div>
          ...
          <div className='flex flex-wrap gap-4 mb-6'>
            <div
              className='rounded-xl px-4 py-3 bg-[#1A1B1E]/10 border border-[#373A40]/20 shadow-sm'
              title='This is how much you need to never work again — once your portfolio hits this number, you can withdraw 4% per year to cover expenses forever'
            >
            ...
            <div
              className='rounded-xl px-4 py-3 bg-[#1A1B1E]/10 border border-[#373A40]/20 shadow-sm'
              title={`${fireType}: ${fireType === "Lean FIRE" ? "Retiring on ≤$50K/yr — you can quit sooner with a frugal lifestyle" : fireType === "Fat FIRE" ? "Retiring on $100K+/yr — comfortable but takes longer to reach" : "Retiring on $50K–$100K/yr — a balanced approach"}. This is when you can stop working entirely.`}
            >
            ...
            {coastFireAge != null && (
              <div
                className='rounded-xl px-4 py-3 bg-[#1A1B1E]/10 border border-[#373A40]/20 shadow-sm'
                title='You can stop saving at this age — your investments will grow on their own to hit your FIRE number by 65. You still work to pay bills, but the pressure is off'
              >
            ...
            {baristaFireAge != null && (
              <div
                className='rounded-xl px-4 py-3 bg-[#1A1B1E]/10 border border-[#373A40]/20 shadow-sm'
                title='You can quit your career at this age — a chill part-time gig plus your investment withdrawals will cover all your expenses'
              >
                <div className='text-xs font-semibold uppercase'>Barista FIRE</div>
                <div className='text-lg font-bold'>Age {baristaFireAge}</div>
                <div className='text-xs text-muted'>
                  Part-time {fmt(partTimeIncome)}/yr + 4% withdrawal covers expenses
                </div>
              </div>
            )}
          </div>
          ...
          {/* Chart */}
          <div
            className='bg-white rounded-xl border border-[#373A40]/30 p-4'
            style={{ height: 480 }}
