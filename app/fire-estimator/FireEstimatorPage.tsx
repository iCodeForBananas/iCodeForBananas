...
function calcProjection(
  ...
  const fireNumber = expenses * FIRE_MULTIPLIER;
  ...
  useEffect(() => {
    if (loaded)
      localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify({
          income, monthlyExpenses, inflation, age, lifeExpectancy,
          investIn, netWorth, partTimeIncome, monthly401k, employerMatchPct, matchCapPct,
        }),
      );
    ...
    <BentoPageLayout title="FIRE Estimator">
      <div className='flex flex-wrap gap-6 mb-6'>
        <label className='flex flex-col gap-1' title='Starting earlier means more years of compound growth — even a few years makes a big difference in when you can retire'>
          ...
          <input type='number' value={age} onChange={(e) => setAge(Number(e.target.value))} className='border border-[#373A40]/30 rounded px-3 py-2 w-28 bg-white' />
        </label>
        <label className='flex flex-col gap-1' title='How long your money needs to last — retiring at 35 with a life expectancy of 90 means 55 years of withdrawals'>
          ...
          <input type='number' value={lifeExpectancy} onChange={(e) => setLifeExpectancy(Number(e.target.value))} className='border border-[#373A40]/30 rounded px-3 py-2 w-28 bg-white' />
        </label>
        <label className='flex flex-col gap-1' title='Higher income means more you can save each year — the gap between this and your expenses is what gets invested toward retirement'>
          ...
          <input type='number' step='1000' value={income} onChange={(e) => setIncome(Number(e.target.value))} className='border border-[#373A40]/30 rounded px-3 py-2 w-48 bg-white' />
        </label>
        <label className='flex flex-col gap-1' title='Money you already have invested — a head start that compounds and can shave years off your retirement date'>
          ...
          <input type='number' step='1000' value={netWorth} onChange={(e) => setNetWorth(Number(e.target.value))} className='border border-[#373A40]/30 rounded px-3 py-2 w-48 bg-white' />
        </label>
        <label className='flex flex-col gap-1' title='The single biggest lever — cutting expenses lowers your FIRE number AND increases how much you save. Reducing by $500/mo can move retirement up by years'>
          ...
          <input type='number' step='100' value={monthlyExpenses} onChange={(e) => setMonthlyExpenses(Number(e.target.value))} className='border border-[#373A40]/30 rounded px-3 py-2 w-48 bg-white' />
        </label>
        <label className='flex flex-col gap-1' title='With Barista FIRE, part-time income fills the gap so you can quit your career years before full FIRE — even a small side gig helps'>
          ...
          <input type='number' step='1000' value={partTimeIncome} onChange={(e) => setPartTimeIncome(Number(e.target.value))} className='border border-[#373A40]/30 rounded px-3 py-2 w-48 bg-white' />
        </label>
        <label className='flex flex-col gap-1' title='401k contributions come from pre-tax income and get invested — maxing this out accelerates your retirement date'>
          ...
          <input type='number' step='100' value={monthly401k} onChange={(e) => setMonthly401k(Number(e.target.value))} className='border border-[#373A40]/30 rounded px-3 py-2 w-36 bg-white' />
        </label>
        <label className='flex flex-col gap-1' title='Free money from your employer — a 50% match means every $1 you contribute adds $1.50 toward retirement'>
          ...
          <input type='number' step='5' value={employerMatchPct} onChange={(e) => setEmployerMatchPct(Number(e.target.value))} className='border border-[#373A40]/30 rounded px-3 py-2 w-28 bg-white' />
        </label>
        <label className='flex flex-col gap-1' title='Your employer only matches up to this % of your salary — contribute at least this much to get the full free money'>
          ...
          <input type='number' step='1' value={matchCapPct} onChange={(e) => setMatchCapPct(Number(e.target.value))} className='border border-[#373A40]/30 rounded px-3 py-2 w-28 bg-white' />
        </label>
        <label className='flex flex-col gap-1' title='Inflation raises your cost of living every year — higher inflation means you need a bigger portfolio to retire, pushing your date out'>
          ...
          <input type='number' step='0.5' value={inflation} onChange={(e) => setInflation(Number(e.target.value))} className='border border-[#373A40]/30 rounded px-3 py-2 w-28 bg-white' />
        </label>
        <div className='flex flex-col gap-1 justify-end'>
          <label className='flex flex-col gap-1' title='Investing in index funds grows your money faster — SPY or QQQ can cut years off your retirement date vs keeping cash'>
            ...
            <select value={investIn} onChange={(e) => setInvestIn(e.target.value)} className='border border-[#373A40]/30 rounded px-3 py-2 bg-white text-sm'>
              <option value='none'>None (cash)</option>
              <option value='spy'>SPY (S&P 500)</option>
              <option value='qqq'>QQQ (Nasdaq-100)</option>
            </select>
          </label>
          <span className='text-sm text-muted' title="The higher your savings rate, the sooner you retire — at 50% you're working one year for every year of freedom, at 75% you retire in roughly 7 years">
            Savings rate: {savingsRate}%
          </span>
          {returnRate > 0 && (
            <span className='text-sm text-muted' title='Your investments compound at this rate each year — the difference between 10% and 13% can move your retirement date by several years'>
      ...
      <div className='flex flex-wrap gap-4 mb-6'>
        ...
        <div className='rounded-xl px-4 py-3 bg-[#1A1B1E]/10 border border-[#373A40]/20 shadow-sm' title={`${fireType}: ${fireType === "Lean FIRE" ? "Retiring on ≤$50K/yr — you can quit sooner with a frugal lifestyle" : fireType === "Fat FIRE" ? "Retiring on $100K+/yr — comfortable but takes longer to reach" : "Retiring on $50K–$100K/yr — a balanced approach"}. This is when you can stop working entirely.`}>
          ...
          <div className='text-lg font-bold'>
            {retireAge != null ? `Retire at ${retireAge} — year ${new Date().getFullYear() + (retireAge - age)}` : "Not reached"}
          </div>
          {retireAge != null && retireYearIdx >= 0 && (
            <div className='text-xs text-muted'>Portfolio at retirement: {fmt(data[retireYearIdx]?.portfolio ?? 0)}</div>
          )}
        </div>
        {coastFireAge != null && (
      ...
      <div className='bg-white rounded-xl border border-[#373A40]/30 p-4' style={{ height: 480 }} title='When the purple portfolio line crosses above the pink FIRE target line, you can retire — the green vertical line marks that age'>
        ...
        <ResponsiveContainer width='100%' height='100%'>
          <AreaChart data={data} margin={{ top: 10, right: 30, left: 10, bottom: 0 }}>
            ...
            {retireAge != null && (
              <ReferenceLine x={retireAge} stroke='#22c55e' strokeDasharray='4 4' label={{ value: `Retire @ ${retireAge}`, position: "top", fill: "#22c55e", fontSize: 12, fontWeight: 600 }} />
            )}
          </AreaChart>
        </ResponsiveContainer>
        </ClientOnly>
