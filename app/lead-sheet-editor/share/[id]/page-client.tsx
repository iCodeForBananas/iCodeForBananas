...
function SheetContent({ sheet }: { sheet: LeadSheet }) {
  return (
    <div>
      <div className='mb-8 border-b-2 border-line-strong border-line-strong pb-6'>
        <h1 className='font-bold leading-tight mb-3 text-[2.25em] text-ink-primary'>
          {sheet.title || "Untitled"}
        </h1>
        <div className='flex flex-wrap gap-6 text-[0.875em]'>
          {sheet.key && (
            <span>
              <span className='uppercase tracking-wider text-[0.75em] text-ink-muted mr-1'>Key</span>
              <span className='font-bold text-ink-primary text-[1em]'>{sheet.key}</span>
            </span>
          )}
          {sheet.tempo && (
            <span>
              <span className='uppercase tracking-wider text-[0.75em] text-ink-muted mr-1'>Tempo</span>
              <span className='font-bold text-ink-primary text-[1em]'>{sheet.tempo} BPM</span>
            </span>
          )}
        </div>
        {sheet.general_notes && (
          <p className='mt-3 italic text-ink-muted text-[0.875em]'>{sheet.general_notes}</p>
        )}
      </div>
        ...
        {sheet.sections.map((section: Section) => {
          ...
          return (
            <div key={section.id} style={{ breakInside: "avoid" }}>
              ...
              {section.notes && (
                <p className='mt-3 italic text-ink-muted text-[0.875em]'>↳ {section.notes}</p>
              )}
            </div>
          );
        })}
...
export default function ShareLeadSheet({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!sheet) {
    return (
      <div className='min-h-screen bg-surface-base flex items-center justify-center'>
        <span className='text-ink-muted text-sm'>Sheet not found.</span>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      {/* Screen view */}
      <div className='print:hidden min-h-screen bg-surface-base'>
        <div className='max-w-3xl mx-auto px-6 pt-8'>
          <div className='flex items-center justify-between mb-8'>
            <span className='text-xs text-ink-muted font-medium tracking-wider uppercase'>
              iCodeForBananas
            </span>
