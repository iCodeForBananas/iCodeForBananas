  ...
  return (
    <div className='flex flex-col flex-1 min-h-0'>
      <main className='flex flex-col flex-1 min-h-0 pl-2 pt-2 pr-0 pb-0 sm:pl-4 sm:pt-4 sm:pr-0 sm:pb-0'>
        <div
          className={`flex flex-col flex-1 min-h-0 rounded-none border-none bg-black overflow-hidden${maxWidth ? ` ${maxWidth} mx-auto` : ""}${boxClassName ? ` ${boxClassName}` : ""}`}
        >
          <div className='shrink-0'>
            <div className='px-4 pt-2 pb-1.5 sm:px-6 sm:pt-3 sm:pb-2.5'>
              <h1
                className={`text-lg sm:text-xl font-bold leading-tight ${titleClassName ?? "text-yellow-400"}`}
              >
                {title}
              </h1>
            </div>
