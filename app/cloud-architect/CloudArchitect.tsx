...
import { SERVICE_METADATA, SCENARIOS, PASS_THRESHOLD, ConfigField } from "./constants";
import { evaluateDesignLocally } from "./services/localEvaluator";
...
export default function CloudArchitect() {
  ...
  const [simulationPhase, setSimulationPhase] = useState<"idle" | "animating" | "results">("idle");

  // Level progression
  const [unlockedLevel, setUnlockedLevel] = useState(1);
  const [showLevelUp, setShowLevelUp] = useState(false);

  const passed = simulationResult ? simulationResult.score >= PASS_THRESHOLD : false;
  ...
  const startSimulation = () => {
    ...
    const result = evaluateDesignLocally(currentScenario, nodes, connections);
    setSimulationResult(result);
    setSimulationPhase("animating");
  };

  const handleTrafficSimComplete = () => {
    setSimulationPhase("results");
    setIsSimulating(false);

    // Advance level if passed and this is the highest unlocked level
    if (
      simulationResult &&
      simulationResult.score >= PASS_THRESHOLD &&
      currentScenario.level >= unlockedLevel &&
      currentScenario.level < SCENARIOS.length
    ) {
      setUnlockedLevel(currentScenario.level + 1);
      setShowLevelUp(true);
    }
  };
  ...
  return (
    <div className='flex h-screen bg-slate-900 overflow-hidden text-slate-100'>
                    ...
                    <div key={field.key} className='flex flex-col gap-1.5'>
                      <div className='flex items-center justify-between'>
                        ...
                        {field.type === "toggle" && (
                          <button
                            onClick={() =>
                              updateNodeConfig(selectedNode.id, field.key, !selectedNode.config[field.key])
                            }
                            className={`w-10 h-5 rounded-full transition-all relative ${selectedNode.config[field.key] ? "bg-orange-600" : "bg-slate-700"}`}
                          >
                            <div
                              className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${selectedNode.config[field.key] ? "left-6" : "left-1"}`}
                            />
                          </button>
                        )}
                      </div>
      ...
      <main className='flex-1 relative flex flex-col min-w-0'>
        ...
        <header className='h-16 bg-slate-800/80 backdrop-blur border-b border-slate-700 flex items-center justify-between px-6 z-10 shrink-0'>
          <div className='flex items-center gap-4'>
            <div className='bg-orange-600/20 border border-orange-500/40 px-3 py-1 rounded-full flex items-center gap-2'>
              <Trophy size={14} className='text-orange-400' />
              <span className='text-xs font-bold text-orange-400'>Level {currentScenario.level}/10</span>
            </div>
        ...
        >
            ...
            <div
              ...
              className={`absolute -translate-x-1/2 -translate-y-1/2 group transition-all duration-200 ${
                draggingNodeId === node.id
                  ...
                  : "z-10 cursor-grab hover:scale-105 active:scale-95"
              } ${connectionStart === node.id ? "z-20" : ""} ${selectedNodeId === node.id ? "ring-4 ring-orange-500 scale-105 z-20" : ""}`}
              style={{ left: node.x, top: node.y }}
            >
              <div
                className={`${SERVICE_METADATA[node.type].color} p-4 rounded-xl shadow-xl ring-4 ring-slate-900 group-hover:ring-orange-500/50 transition-all relative`}
              >
              ...
              <div
                className={`absolute -bottom-8 left-1/2 -translate-x-1/2 bg-slate-800 text-[10px] font-bold px-2 py-0.5 rounded border ${selectedNodeId === node.id ? "border-orange-500" : "border-slate-700"} whitespace-nowrap opacity-80 group-hover:opacity-100`}
              >
                {node.label}
              </div>
            </div>
            ...
            <div className='absolute inset-0 flex flex-col items-center justify-center text-slate-500 pointer-events-none'>
              ...
              <div className='mt-6 flex gap-4 text-xs'>
                <span className='flex items-center gap-1 bg-slate-800 px-3 py-1 rounded-full'>
                  <Settings2 size={12} /> Toggles for OAC, Multi-AZ, etc.
                </span>
                <span className='flex items-center gap-1 bg-slate-800 px-3 py-1 rounded-full'>
                  <Plus size={12} /> Click to place
                </span>
                <span className='flex items-center gap-1 bg-slate-800 px-3 py-1 rounded-full'>
                  <Repeat size={12} /> Connect layers
                </span>
              </div>
            </div>
          )}
          ...
          {simulationPhase === "animating" && simulationResult && (
            <TrafficSimulator
              nodes={nodes}
              connections={connections}
              score={simulationResult.score}
              targetRPS={currentScenario.targetRPS}
              onComplete={handleTrafficSimComplete}
            />
          )}

          {/* Floating Result Panel */}
          {simulationPhase === "results" && simulationResult && (
            <div className='absolute top-6 right-6 bottom-6 w-[450px] bg-slate-800/95 backdrop-blur-md rounded-2xl border border-slate-700 shadow-2xl z-20 flex flex-col overflow-hidden'>
              <div className='p-6 border-b border-slate-700'>
                <div className='flex justify-between items-center mb-3'>
                  <div>
                    ...
                    <p className='text-xs text-slate-400'>
                      Level {currentScenario.level} &mdash; {currentScenario.title}
                    </p>
                  </div>
                  <div
                ...
                >
                  ...
                  {passed
                    ? currentScenario.level < SCENARIOS.length
                      ? `Passed! Level ${currentScenario.level + 1} unlocked.`
                      : "Passed! You completed all 10 levels!"
                    : `Score ${PASS_THRESHOLD}+ required to pass. Try again!`}
                </div>
              </div>
                ...
                <section>
                  ...
                  <ul className='space-y-2'>
                    {simulationResult.bottlenecks.map((b, i) => (
                      <li
                        key={i}
                        className='text-xs flex items-center gap-2 bg-red-500/10 border border-red-500/20 p-2 rounded text-red-300'
                      >
              ...
              <div className='p-6 bg-slate-900/50 border-t border-slate-700 space-y-2'>
                {passed && currentScenario.level < SCENARIOS.length && (
                  <button
                    onClick={() => {
                      const next = SCENARIOS.find((s) => s.level === currentScenario.level + 1);
                      if (next) {
                        setCurrentScenario(next);
                        resetGame();
                      }
                    }}
                    className='w-full py-2 bg-green-600 hover:bg-green-500 rounded text-sm font-bold transition flex items-center justify-center gap-2'
                  >
                    <ChevronRight size={16} /> Next Level
                  </button>
                )}
                <button
                  onClick={() => {
                ...
                >
                  {passed ? "Review Design" : "Return to Drafting"}
                </button>
              </div>
            </div>
          )}
        ...
        <div className='fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-6'>
          <div className='bg-slate-800 rounded-3xl border border-slate-700 max-w-xl w-full shadow-2xl overflow-hidden'>
            ...
            <div className='p-8'>
              ...
              <p className='text-slate-300 leading-relaxed text-lg mb-8'>{currentScenario.description}</p>
              ...
              <div className='grid grid-cols-2 gap-6 p-6 bg-slate-900/50 rounded-2xl border border-slate-700'>
                <div>
                  ...
                  <div className='text-xl font-bold text-white'>
                    {currentScenario.targetRPS.toLocaleString()}{" "}
                    <span className='text-sm font-medium text-slate-400'>req/min</span>
                  </div>
                </div>
                <div>
                  <div className='text-[10px] text-slate-500 uppercase font-bold tracking-widest mb-1'>
                  ...
                  <div className='text-xl font-bold text-white'>
                    ${currentScenario.budget} <span className='text-sm font-medium text-slate-400'>/hr</span>
                  </div>
                </div>
                <div>
                  <div className='text-[10px] text-slate-500 uppercase font-bold tracking-widest mb-1'>
      ...
      {/* Scenario Picker Modal (Level Select) */}
      {showScenarioPicker && (
        <div className='fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-6'>
          <div className='bg-slate-800 rounded-3xl border border-slate-700 max-w-2xl w-full shadow-2xl overflow-hidden max-h-[90vh] flex flex-col'>
            <div className='p-8 border-b border-slate-700 bg-gradient-to-br from-orange-600/20 to-transparent shrink-0'>
              <h2 className='text-3xl font-black mb-2 flex items-center gap-3'>
                <Trophy className='text-orange-500' size={28} /> Select Level
              </h2>
              <p className='text-slate-400 mb-4'>
                Complete each level with a score of {PASS_THRESHOLD}+ to unlock the next.
              </p>
              {/* Progress bar */}
              <div className='flex items-center gap-3'>
                ...
                <span className='text-xs font-bold text-slate-400 whitespace-nowrap'>
                  {unlockedLevel - 1}/{SCENARIOS.length} cleared
                </span>
              </div>
            </div>
            <div className='p-8 grid gap-3 overflow-y-auto overflow-x-hidden flex-1'>
              {SCENARIOS.map((s) => {
                const isLocked = s.level > unlockedLevel;
                const isCleared = s.level < unlockedLevel;
                return (
                  <button
                    key={s.id}
                    disabled={isLocked}
                    ...
                    className={`group flex items-center justify-between p-5 rounded-2xl border transition-all text-left ${
                      isLocked
                        ...
                        : isCleared
                          ...
                          : "border-slate-700 bg-slate-900/50 hover:border-orange-500 hover:bg-orange-500/5"
                    }`}
                  >
                    <div className='flex items-center gap-4 flex-1 min-w-0'>
                      ...
                      >
                        {isLocked ? <Lock size={16} /> : isCleared ? <CheckCircle2 size={16} /> : s.level}
                      </div>
                      <div className='min-w-0'>
                        <div className='flex items-center gap-2 mb-0.5'>
                          <h3
                            className={`text-sm font-bold transition-colors truncate ${
                              isLocked ? "text-slate-500" : "group-hover:text-orange-500"
                            }`}
                          >
                            {s.title}
                          </h3>
                        ...
                        <div className='flex gap-4 mt-1.5'>
                          <span className='text-[10px] text-slate-500 uppercase font-bold tracking-wider'>
                            {s.targetRPS.toLocaleString()} RPM
                          </span>
                          <span className='text-[10px] text-slate-500 uppercase font-bold tracking-wider'>
                            ${s.budget} budget
                          </span>
                    ...
                    <div className='shrink-0 ml-2'>
                      {isLocked ? (
                        ...
                        <ChevronRight
                          size={18}
                          className={`transition-colors ${isCleared ? "text-green-500" : "text-slate-700 group-hover:text-orange-500"}`}
                        />
                      )}
                    </div>
                  </button>
            ...
            <div className='px-8 pb-6 shrink-0'>
              <p className='text-xs text-center text-slate-500 italic'>
                Score {PASS_THRESHOLD}+ to pass each level. Difficulty increases with each stage.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className='fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[60] flex items-center justify-center p-6'>
          <div className='bg-slate-800 rounded-3xl border border-orange-500/50 max-w-md w-full shadow-2xl overflow-hidden text-center'>
            <div className='p-10 bg-gradient-to-br from-[#4C6EF5]/20 via-[#12B886]/10 to-transparent'>
              ...
              <h2 className='text-3xl font-black mb-2 text-orange-400'>Level Up!</h2>
              <p className='text-slate-300 text-lg mb-2'>You passed Level {currentScenario.level}</p>
              <p className='text-slate-400 text-sm'>
                {currentScenario.level < SCENARIOS.length
                  ? `Level ${currentScenario.level + 1}: ${SCENARIOS[currentScenario.level]?.title} is now unlocked!`
                  : "You have completed all 10 levels. You are a true Cloud Architect!"}
              </p>
            </div>
            <div className='p-6 space-y-3'>
              {currentScenario.level < SCENARIOS.length && (
                <button
                  ...
                  className='w-full py-3 bg-orange-600 hover:bg-orange-500 rounded-xl text-sm font-bold transition flex items-center justify-center gap-2'
                >
                  <ChevronRight size={16} /> Start Level {currentScenario.level + 1}
                </button>
              )}
              <button
                onClick={() => setShowLevelUp(false)}
                className='w-full py-2 bg-slate-700 hover:bg-slate-600 rounded-xl text-sm font-bold transition'
              >
                Stay Here
              </button>
            </div>
