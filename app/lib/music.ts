...
// Build a list of chord names for a key + preset
export function buildProgressionChords(keyRoot: string, preset: ProgressionPreset): string[] {
  const rootIndex = allNotes.indexOf(keyRoot);
  if (rootIndex < 0) return [];

  // Degree-based (explicit quality)
  ...
  // Pattern-based (heuristic quality)
