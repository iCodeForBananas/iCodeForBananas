...
const getRootPositions = (note: string, voicing: Voicing): InversionShape[] => {
  ...
  for (const n of [note, canonical, enharmonic].filter(Boolean) as string[]) {
    ...
    if (s?.[0]) {
      const k = s[0].frets.join(",");
      if (!seen.has(k)) {
        seen.add(k);
        shapes.push({ shape: s[0], label: "Open/Standard" });
      }
      break;
    }
  }
  ...
  if (eT) {
    const s = transposeShape(eT, semitoneFromE(note));
    if (s) {
      const k = s.frets.join(",");
      if (!seen.has(k)) {
        seen.add(k);
        shapes.push({ shape: s, label: "E-Shape Barre" });
      }
    }
  }
  // A-shape barre
  ...
  if (aT) {
    const s = transposeShape(aT, semitoneFromA(note));
    if (s) {
      const k = s.frets.join(",");
      if (!seen.has(k)) {
        seen.add(k);
        shapes.push({ shape: s, label: "A-Shape Barre" });
      }
    }
  }
  ...
  TEMPLATES[voicing].root.forEach((t, i) => {
    ...
    if (!seen.has(k)) {
      seen.add(k);
      shapes.push({ shape: s, label: STRING_SET_LABELS[i] });
    }
  });
  return shapes;
};
                  ...
                  <div className='flex flex-wrap gap-2'>
                    ...
                    <button
                      onClick={() => {
                        ...
                        setUseFlats(newVal);
                        if (newVal) {
                          const f = sharpToFlat[selectedNote];
                          if (f) setSelectedNote(f);
                        } else {
                          const s = flatToSharp[selectedNote];
